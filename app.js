...
// ===== 2. EQUAÇÕES (6 fórmulas com \frac) =====
...
const BUFFER_SINAL_SEG = 180;
const TOTAL_COMBINACOES = 864;
...
function alternarDicaFase(questId, faseNum) {
  ...
  if (boxDica) {
    ...
    const dados = getDadosQuestao(questId);
    dicasUsadas = Math.max(dados.dicasUsadas || 0, faseNum);
    dados.dicasUsadas = dicasUsadas;
    salvarStorage();
  }
}
...
function aplicarZoom(nivel) {
  const scale = Math.max(0.8, Math.min(4.0, nivel));
  ESTADO.zoomLevel = scale;
  document.documentElement.style.setProperty('--enunciado-scale', scale);
  salvarStorage();
...
function aplicarZoomEquacoes(nivel) {
  ...
  document.documentElement.style.setProperty('--eq-scale', scale);
  salvarStorage();
  const ind = document.getElementById('zoom-eq-indicador');
...
async function sincronizarNTP() {
  const apis = [
    { url: 'https://worldtimeapi.org/api/timezone/America/Sao_Paulo', parse: (data) => new Date(data.datetime).getTime() },
    { url: 'https://timeapi.io/api/time/current/zone?timeZone=America/Sao_Paulo', parse: (data) => new Date(data.dateTime || data.datetime).getTime() }
  ];
...
function calcularProvaSala() {
  ...
  const meiaNoiteMs = new Date(agora.getFullYear(), agora.getMonth(), agora.getDate()).getTime();
  ...
  const infoBase = {
    ...
    tempoTotal, sinalMs, fechamentoMs,
    calculadoEmMs: agoraMs
  };

  if (tempoTotal < MINIMO_VIAVEL) {
    return { ...infoBase, valido: false, motivo: 'tempo_insuficiente',
  ...
  if (tempoTotal <= ALERTA_MAXIMO) {
    ...
    return { ...infoBase, valido: true, modo: 'reduzido_2questoes',
      etapas: [
...
function renderizarQuadroEquacoes() {
  ...
  const htmlFormulas = DATABASE_EQUACOES.map(eq =>
    `<div class="card-formula"><div style="display:flex; justify-content:space-between; align-items:center;"><span class="tag-tipo">${eq.badge}</span><strong style="font-size: 1.05rem; color: var(--ink);">${eq.concept}</strong></div><div class="formula-render">$${eq.katex}$</div><div style="font-size:0.85rem; color:#495057;">${eq.desc}</div></div>`
  ).join('');
...
function montarPainelInfoSala(container, resultadoSala) {
  ...
  painelInfo.innerHTML = `
    <div style="background:#f1f3f5; border-left:4px solid #173fa6; padding:12px 14px; border-radius:6px; font-size:0.95rem; margin:12px 0; text-align:left;">
      ...
      <div><strong>⏱️ Tempo real disponível agora:</strong> <span id="preview-tempo-real" style="font-family:'Fira Code',monospace; font-weight:bold; color:#173fa6;">${formatarSegundos(resultadoSala.tempoTotal)}</span></div>
      ...
      <div style="margin-top:8px; color:#495057; font-style:italic;">
        💡 Ao clicar em INICIAR, o tempo é recalculado com o relógio atual.
      </div>
    </div>`;
  ...
  window._timerPreviewSala = setInterval(() => {
    ...
    if (!modal || modal.classList.contains('oculto')) {
      clearInterval(window._timerPreviewSala);
      window._timerPreviewSala = null;
      return;
    }
    const el = document.getElementById('preview-tempo-real');
    if (!el) return;
    const r = calcularProvaSala();
    el.textContent = r.valido ? formatarSegundos(r.tempoTotal) : '—';
    el.style.color = (r.valido && r.tempoTotal < 15 * 60) ? '#e03131' : '#173fa6';
  }, 1000);
}
...
function iniciarProvaComSeed(seed, modo, resultadoSala) {
  ...
  if (modo === 'sala') {
    ...
    fechamentoProvaTimestamp = resultadoSala.fechamentoMs;
  } else {
    etapasProvaAtual = [
...
function pularTempo() {
  ...
  console.log(`⏭️ [TESTE] Pulando etapa "${etapaOrigem}". Restavam ${tempoRestanteEtapa}s.`);
  tempoRestanteEtapa = 0;
  avancarProximaEtapaAvaliacao();
}
window.pularTempo = pularTempo;
...
function atualizarInterfaceAvaliacao() {
  ...
  if (nivelAtual === 'revisao') container.classList.add('modo-revisao');
  ...
  if (nivelAtual === 'revisao') {
    ...
    questoes.forEach((q, idx) => {
      renderizarQuestaoCardSimulado(q, container, idx + 1, rotulos[idx], true);
    });
  } else {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnImprimirGab) {
    btnImprimirGab.addEventListener('click', () => {
      const boxGab = document.getElementById('gabarito-pos-prova');
      if (boxGab && boxGab.classList.contains('oculto')) {
        const btnVer = document.getElementById('btn-ver-gabarito-simulado');
  ...
  document.addEventListener('click', (e) => {
    if (e.target.closest('#aba-equacoes-prova')) {
      ...
      if (drawer) drawer.classList.toggle('aberto');
    }
    if (e.target.closest('#btn-fechar-painel-eq')) {
      const drawer = document.getElementById('painel-equacoes-prova');
